...
from ui.detect_ui2 import Ui_MainWindow # 导入detect_ui的界面
...
class UI_Logic_Window(QtWidgets.QMainWindow):
    ...
    def init_slots(self):
        ...
        self.ui.pushButton_plc.clicked.connect(self.plc_connect)
    ...
    def plc_connect(self):
        ...
        #提示框打印
        MessageBox(title='提示', text='正在与PLC建立通讯.......', time=2000, auto=True).exec_()
        #第二个信息通知框
        MessageBox(title='成功！', text='PLC通讯成功！！！', time=4000, auto=True).exec_()

        '''
        20220912:逻辑说明
        此处预留一个程序api接口，此接口需要与PLC通讯
        通讯逻辑如下：
            1、点击PLC通讯按钮后第一个信息通知框先发送
            2、程序开始连接PLC的以太网或者其他协议通信
            3、程序发送测试数据后，PLC返回一个标志
            4、接收到标志后，证明程序已经和PLC建立了通讯
            5、第二个信息通知框发送
        '''
            ...
            for i, det in enumerate(pred):
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        # print(single_info)
                        str = single_info.split(":")[1]
                        str2 = str.split(")")[0]
                        str3 = str2.split("(")[1]
                        str4 = str3.split(",")[0]
                        str5 = str3.split(",")[1]
                        str6 = float(str4)#中心x轴
                        str7 = float(str5)#中心y轴
                        ...
                        if ii:                   
                            ...
                            center.append(str6)
                            #print(center)
                            i1 = center[1]-center[0]
                            center1.append(i1)
                            #v = abs(center1[0])#绝对值
                            v = center1[0]
                            s = "X轴相对距离：%0.1f"%v
                            info_show = info_show + s +'\n'
                            #print("X轴相对距离：%0.1f"%v)
